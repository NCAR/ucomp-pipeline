...
pro ucomp_write_iquv_mp4, wave_region, run=run
  ...
  mg_log, 'writing IQUV mp4 for %s nm', wave_region, name=run.logger_name, /info

  perform_centering = run->config('centering/perform')
  if (~perform_centering) then begin
    mg_log, 'skipping writing IQUV mp4 because not centering images', $
  ...
  mg_log, 'writing IQUV mp4 %s for %s nm', mp4_filename, wave_region, $
