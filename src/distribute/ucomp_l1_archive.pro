...
pro ucomp_l1_archive, wave_region, run=run
  ...
  if (~run->config(wave_region + '/send_to_archive')) then begin
    mg_log, 'skipping archiving %s nm L1 data [TODO]', wave_region, $
  ...
  ; skip sending L1 to archive if tarball already exists
  if (file_test(tarfile, /regular)) then begin
    mg_log, '%s nm level 1 tarball already exists, skipping', wave_region, $
  ...
  types = [string(wave_region, format='*.ucomp.%s.*.fts*'), $
           string(wave_region, format='*.ucomp.%s.*.log'), $
  ...
  if (n_files_by_type[0] eq 0L) then begin
    mg_log, 'no %s nm level 1 FITS files to tar', wave_region, $
            name=run.logger_name, /warn
    goto, done
  endif
  ...
  ; put link to level 1 tarball in the archive queue directory
