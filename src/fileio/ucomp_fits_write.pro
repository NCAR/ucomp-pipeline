...
pro ucomp_fits_write, file_or_fcb,data, $
  ...
  if (fcbtype eq 7) then begin
    ...
    fits_open, file_or_fcb, fcb, /write, no_abort=no_abort, message=message
  ...
  if (n_elements(xtension) gt 0) then begin
    ...
    if (N_Axtension eq 0) then Axtension = ''
 	endelse
  if (Axtension eq 'BINTABLE' or (Axtension eq 'TABLE')) then begin
  ...
  if (n_elements(extlevel) gt 0) then begin
    Aextlevel = extlevel
  endif else Aextlevel = sxpar(header, 'extlevel')

  sxdelpar, header, ['XTENSION', 'EXTNAME', 'EXTVER', 'EXTLEVEL']

  ; separate header into main and extension header
  keywords = strmid(header,0,8)
  hpos1 = where(keywords eq 'BEGIN MA')
  hpos1 = hpos1[0]    ; begin main
  hpos2 = where(keywords eq 'BEGIN EX')
  hpos2 = hpos2[0]   ; begin ext
  hpos3 = where(keywords eq 'END     ')
  hpos3 = hpos3[0]   ; end of header
  ...
  if (fcb.nextend eq -1) then begin   ; no pdu written yet?
    if ((hpos2 gt 0) || (Axtension ne '') || (Aextname ne '') || $
      ...
      if (hpos1 gt 0) && (hpos2 gt (hpos1+1)) then begin
  ...
  message,' ERROR: '+ message, /continue
  retall
end
