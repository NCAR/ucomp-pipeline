...
pro ucomp_average_l1_files, files, output_filename, method=method, run=run
  ...
  all_data = make_array(dimension=[dims[0:2], n_unique_wavelengths, n_files], $
                        type=size(ext_data, /type)) + !values.f_nan
  n_extensions = lonarr(n_unique_wavelengths)
  ...
  for f = 0L, n_files - 1L do begin
    ...
    for e = 0L, n_extensions - 1L do begin
      ...
      all_data[0, 0, 0, wavelength_match_indices[0], f] = ext_data[*, *, *, e]
      n_extensions[wavelength_match_indices[0]] += 1L
     endfor
  ...
  case _method of
    'mean': averaged_data = mean(all_data, dimension=5, /nan)
    'median': averaged_data = median(all_data, dimension=5)
