...
function ucomp_validate_dates_expandrange, start_date, end_date, count=n_days
...
pro ucomp_validate_dates, date_expression, config_filename=config_filename
  compile_opt strictarr
  on_error, 2

  mg_log, name='ucomp/validate', logger=logger
  ...
  for r = 0L, n_ranges - 1L do begin
    ...
    case n_endpts of
      ...
      1: begin
          ucomp_validate, endpts[0], config_filename, status=status
          if (status ne 0L) then failed_days->add, endpts[0]
          mg_log, name='ucomp/validate', logger=logger
          logger->setProperty, format='%(time)s %(levelshortname)s: %(message)s'
          mg_log, divider, name='ucomp/validate', /info
        end
      2: begin
          dates = ucomp_validate_dates_expandrange(endpts[0], endpts[1], $
                                                 count=n_dates)
          for d = 0L, n_dates - 1L do begin
            ucomp_validate, dates[d], config_filename=config_filename, status=status
  ...
  if (n_failed_days gt 0L) then begin
    mg_log, 'failed days: %s', strjoin(failed_days->toArray(), ', '), $
            name='ucomp/validate', /info
  endif else begin
    mg_log, 'no failed days', name='ucomp/validate', /info
  endelse
