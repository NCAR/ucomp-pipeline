...
;               called from GET_SUNSPICE_COORD.
;
; Syntax      :	CONVERT_SUNSPICE_GSE2GSM, DATE, COORD
;
...
;
;                               ERRMSG = ''
;                               State = CONVERT_SUNSPICE_GSE2GSM( ERRMSG=ERRMSG, ... )
;                               IF ERRMSG NE '' THEN ...
;
...
;               Version 8, 26-Apr-2016, WTT, renamed from STEREO_GSE2GSM
;               Version 9, 29-Jun-2016, WTT, added ITRF93 keyword
;
; Contact     :	WTHOMPSON
;-
;
pro convert_sunspice_gse2gsm, date, coord, cmat=cmat, inverse=inverse, itrf93=itrf93, $
                              errmsg=errmsg
;
on_error, 2
if n_params() ne 2 then begin
    message = 'Syntax:  Result = CONVERT_SUNSPICE_GSE2GSM( DATE, COORD )'
    goto, handle_error
endif
;
sz = size(coord)
...
if n_elements(errmsg) eq 0 then message, message else $
  errmsg = 'CONVERT_SUNSPICE_GSE2GSM: ' + message
;
end
