...
;	26-Jan-2012 (Aki Takeda) - Modified to accept Hinode FITS header
...
if (typ eq 8) then begin
    tags = tag_names(item)
    case 1 of
    (tags(0) eq 'GEN') : out = item.gen.time           ; Yohkoh XDA index
    (tags(0) eq 'SIMPLE') and  tag_exist(item,'telescop') :  $
                if item(0).telescop eq 'HINODE' then begin  ; Hinode FITS
                      ...
                      out = work.time
                endif else out = item.time                  ; Yohkoh FITS
    else: out = item.time                             ; originally string
    endcase
    qstruct = 1
end else begin
